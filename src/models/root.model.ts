...
export interface IRootModel {
    ...
    exportFileXlsx(fields: any, name: any): Promise<number>;
...
export class RootModel<S extends ISchema> implements IRootModel {
    ...
    public create = (params: any): Promise<any> => {
        ...
        return new Promise(resolve => {
            ...
            model.save((err, result) => {
                if (err && err.isBoom) resolve(err);
                else {
                    if (err) resolve(Boom.conflict(err));
                    else resolve(result);
                }
            });
        });
     ...
     * update
     */
    public update = async (params: any): Promise<any> => {
        ...
        return new Promise(resolve => {
            this.model.findOneAndUpdate(
                ...
                data,
                err => {
                    if (err && err.isBoom) return resolve(err);
                    if (err) resolve(this.handlerQueryError(err));
    ...
    public findById = (id: string): Promise<any> => {
        ...
        return new Promise(resolve => {
            this.model.findById(_id, (err, result) => {
                if (err) resolve(Boom.notFound("Have Error with database"));
     ...
     * @param populate
     */
    findAll(params: any, populate?: any): Promise<any> {
        ...
        // Query name with include
        if (query.name) query.name = new RegExp(query.name, "i");

        // Query name with include
        if (query.username) query.username = new RegExp(query.username, "i");

        // Query desc with include
        if (query.desc) query.desc = new RegExp(query.desc, "i");

        // Query name with include
        if (query.title) query.title = new RegExp(query.title, "i");

        if (query.subject) query.subject = new RegExp(query.subject, "i");

        return new Promise(resolve => {
            if (_.isUndefined(populate)) {
                this.model
                    .find({ ...query, isRemove: _.isUndefined(query.isRemove) ? false : query.isRemove })
                    .limit(limit)
                    .skip(skip)
                    .sort({
                        $natural: -1
    ...
    getAll({ populate }: any): Promise<any> {
        const fnName = "getAll";
    ...
    async exportFileXlsx(fields: any, name: string): Promise<any> {
    ...
    async countAll({ query }): Promise<any> {
