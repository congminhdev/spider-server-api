...
class UserService extends RootService<IModel> implements IUserService {
    ...
    register = async (user: IUser): Promise<any> => {
        ...
        if (result.isBoom) return result;
        else {
    ...
    changePassword = async (params): Promise<any> => {
        const { data, query } = params;
        const { oldPassword, password } = data;
        this.fnName = "changePassword";
        let user, result;

        LogManager.logInfo(this.className, this.fnName, "Function change user password is calling");

        if (!oldPassword) return Boom.badData(Error.EM0101);

        if (!password) return Boom.badData(Error.EM0104);
        ...
        if (!user) {
            LogManager.logWarn(this.className, this.fnName, `Find user fail`);
        ...
        if (!user.validatePassword(oldPassword)) {
            LogManager.logWarn(this.className, this.fnName, `User  ${user.username} validate password fail`);
        ...
        if (result.isBoom) {
            LogManager.logWarn(this.className, this.fnName, `Update user ${query.id} is fail`);
            return result;
        }
    ...
    forgotPassword = async (params): Promise<any> => {
        const { email } = params;
        this.fnName = "forgotPassword";
        let user, result, token;

        LogManager.logInfo(this.className, this.fnName, "Function forgot password is calling");

        if (!email) {
            return Boom.badData(Error.EM0112);
        }

        user = await this.rootModel.findOne({ email });
        user = user[0];
        ...
        if (result.isBoom) {
            LogManager.logWarn(this.className, this.fnName, `Update user ${user.id} is fail`);
            return "";
        }
    ...
    resetPassword = async (params): Promise<any> => {
        const { data, query } = params;
        const { password } = data;
        this.fnName = "resetPassword";
        let user, result;

        LogManager.logInfo(this.className, this.fnName, "Function reset password is calling");
        ...
        user = user[0];
        if (!user) {
            LogManager.logWarn(this.className, this.fnName, `Password reset token is invalid or has expired.`);
