...
//
import { LogManager, RedisManger } from '../libs';
...
class RootHandler<T extends IService> implements IRouting {
    ...
    protected create = (req: express.Request): any => {
        ...
        return this.rootService.create({ ...req.body, sessionID: req.sessionID, decode: req.decode });
    ...
    protected get = async (req: express.Request): Promise<any> => {
        ...
        LogManager.logInfo(this.className, fnName, 'Function is calling');
        result = await this.rootService.findOne({ _id: id, sessionID: req.sessionID, decode: req.decode })
    ...
    protected delete = async (req: express.Request): Promise<any> => {
        ...
        result = await this.rootService.delete({ id });
    ...
    protected getAll = (req: express.Request): any => {
        ...
        return this.rootService.getAll({ ...req.query, sessionID: req.sessionID, decode: req.decode });
    }
    ...
    protected loginFailHandler = async (id) => {
        ...
        let totalLoginFail: any = await RedisManger.getRedisKey(key) || 0;

        if (totalLoginFail < Const.LIMIT_NUMBER_LOGIN_FAIL) {
            RedisManger.setKey(key, +totalLoginFail + 1, Const.LOGIN_FAIL_EXPIRE_TIME)
        } else {
            RedisManger.setKey(keyDisable, true, Const.LOGIN_FAIL_EXPIRE_TIME)
        }
    }
